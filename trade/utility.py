...
from trade.object import BarData, TickData
from trade.constant import Exchange, Interval, ZH_TRANS_MAP
...
TRADER_DIR, TEMP_DIR = _get_trader_dir("config")
...
def trans_setting(setting: dict):
    new_setting = {}
    for name, value in setting.items():
        if value in ZH_TRANS_MAP.keys():
            new_setting[name] = ZH_TRANS_MAP[value]
        else:
